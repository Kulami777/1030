...
function setup() {
    ...
    // 初始化第一個選項順序
    currentOptionOrder = shuffle(['A', 'B', 'C'], true);
}
...
function displayQuiz() {
    ...
    // 繪製分數 (固定在右上方)
    textSize(optionTextSize * 0.75);
    fill(238, 238, 238, 150);
    text(`目前分數: ${score} / ${totalPoints}`, width - width * 0.1, height * 0.05);
}
...
function mousePressed() {
    if (quizState === 'QUIZ' && currentQuestionIndex < questions.length) {
        ...
        questions = shuffle(questions, true); 
        currentOptionOrder = shuffle(['A', 'B', 'C'], true); // 重新開始也打亂選項
